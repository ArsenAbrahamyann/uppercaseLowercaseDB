...
public class UppercaseLowercase {
    ...
    public static void displayInfoFromMySQL() {
        try {
            Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/mysql_container?createDatabaseIfNotExist=true", "root", "rootPassword");
//            try {
...
//            }
    ...
    public static void displayInfoFromPostgreSQL() {
        try {
            Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres?createDatabaseIfNotExist=true", "postgres", "postgres");
    ...
    public static void createDatabases() {
        try {
            mysqlConnection = DriverManager.getConnection("jdbc:mysql://localhost:3306/mysql_container?createDatabaseIfNotExist=true", "root", "rootPassword");
            postgresConnection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres?createDatabaseIfNotExist=true", "postgres", "postgres");
            ...
            try {
                mysqlStatement.executeUpdate("CREATE TABLE IF NOT EXISTS lowercase (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255))");
    ...
    public static void insertWordsIntoMySQL() {
        try {
            PreparedStatement preparedStatement = mysqlConnection.prepareStatement("INSERT INTO lowercase(name) VALUES (?)");
    ...
    public static void insertWordsIntoPostgreSQL() {
        try {
            postgresConnection.setAutoCommit(false);
            PreparedStatement preparedStatement = postgresConnection.prepareStatement("INSERT INTO uppercase(name) VALUES (?)");
            for (String word : lowercaseWords) {
                preparedStatement.setString(1, word);
                preparedStatement.addBatch();
            }
